...
Inspired by APT15 (Ke3chang)'s pioneering use of DNS tunneling for covert C2 communication — specifically their RoyalCli (RoyalDNS) backdoor which used DNS queries to establish stealthy command channels — NEPHILM demonstrates how modern rootkits can leverage encrypted DNS protocols to evade network detection. APT15 has been active since 2010 and remains one of the most sophisticated China-nexus espionage groups, consistently favoring DNS-based C2 channels to blend with legitimate network traffic and bypass traditional firewalls.
